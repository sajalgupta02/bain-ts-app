...
const GoalCard = ({ index, category, singleGoal }: GoalCardProps) => {
  const { goals, setGoals } = useContext(MainContext);
  ...
  const deleteGoal = () => {
    ...
    for (let data of goals) {
      if (data.category !== category) {
        ...
        const updated = data.goals.filter((_: any, i: number) => i !== index);
        const payload = {
      ...
      <CardContent>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          ...
          <Box display={"flex"} gap={1.5}>
            ...
            <Box sx={{ cursor: "pointer" }} onClick={() => deleteGoal()}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="25"
                height="24"
              ...
              >
                <path
                  d="M9.5 9H11V18H9.5V9ZM14 9H15.5V18H14V9Z"
                  fill="#0064FF"
                />
                <path
        ...
        >
          <Box display={"flex"}>
            ...
            <Typography
              style={{ color: singleGoal.score >= 8 ? "green" : "orange" }}
            >
              {singleGoal.score}/10
            </Typography>
          </Box>
